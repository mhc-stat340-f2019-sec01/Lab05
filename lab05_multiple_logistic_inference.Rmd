...
```{r}
fit_full <- glm(
  judge_decision ~ judge + independent_decision + case_language + claim_location + logit_success, 
  data = refugees, 
  family = 'binomial')
...
```{r}
fit_base <- glm(
  judge_decision ~ judge + independent_decision + case_language + logit_success, 
  data = refugees, 
  family = 'binomial')
summary(fit_base)
```
...
```{r}
fit_3 <- glm(
  judge_decision ~ independent_decision + case_language + logit_success, 
  data = refugees, 
  family = 'binomial')
summary(fit_3)
```
...
If logit_success increases by one unit, the log odds of leave granted to appeal the decision of the Refugee Board increases by 1.3005. 
...
No. The nested ANOVA test suggests that the judge plays a vital role in the odds of leave granted to appeal. This means that the court is by no means just ---- whether I get my leave or not depends largely on the arbitrary odds of who is looking into my case. 
